...
export async function searchDocs(keyword: string): Promise<Block[]> {
    ...
    const sqlScript = `
        ...
        limit 8
    `;
    return sql(sqlScript);
}

export async function queryKeepCards(rootId: string, keyword = ""): Promise<Block[]> {
    if (!rootId) {
        return [];
    }
    const escapedRoot = escapeSql(rootId);
    const trimmed = keyword.trim();
    const keywordSql = trimmed
        ? `and (content like '%${escapeSql(trimmed)}%' or markdown like '%${escapeSql(trimmed)}%' or ial like '%${escapeSql(trimmed)}%')`
        : "";
    const sqlScript = `
        select * from blocks
        where root_id = '${escapedRoot}'
          and (
            ial like '%custom-memo-card="true"%'
            or ial like '%custom-telegram-memo="true"%'
          )
          ${keywordSql}
